...
/**
 * The transition dynamics for the Space invader game
 */
public class SpaceInvaderMechanics extends JointActionModel {
    static final Logger logger = Logger.getLogger(SpaceInvaderMechanics.class.getName());
    private final Random rnd;
    private final Domain domain;

    public SpaceInvaderMechanics(Domain d, Integer randomSeed) {
    ...
    private void freezePlayer(ObjectInstance ship) {
        if (isFrozen(ship)) {
            logger.getRootLogger().warn("Hmm. Trying to freeze an already frozen ship. This should not happen");
        }
        //Move off the map
        ship.setValue(X, -3);
        //Reduce lives by 1
