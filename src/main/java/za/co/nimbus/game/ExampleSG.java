...
public class ExampleSG {
    ...
    public ExampleSG(){
        ...
        this.initialState = GridGame.getTurkeyInitialState(this.domain);
        //this.initialState = GridGame.getPrisonersDilemmaInitialState(this.domain);
    ...
    public void MAValueIteration(){
        ...
        //Once co-operates, the other doesn't
        MAQSourcePolicy ja0 = new EGreedyJointPolicy(0.0);
        MAQSourcePolicy ja1 = new EGreedyMaxWellfare(0.0, false);
...
//        this.w.addWorldObserver(wob);
        ...
        for (int i=0; i<10; i++){
            ...
            v.updateState(this.initialState);

            //run the game.
            GameAnalysis ga = w.runGame();
            System.out.println("\nGame: " + i);
            int t = ga.numTimeSteps()-1;
            System.out.println("Agent0: " + ga.getRewardForAgent(t,a0.getAgentName()));
            System.out.println("Agent1: " + ga.getRewardForAgent(t,a1.getAgentName()));
            ga.writeToFile(String.format("sgVL/%4d", i), sp);

        }

        //now load the game sequence visualizer so we can review what happened during learning
        new GameSequenceVisualizer(v, domain, sp, "sgVL/");
    ...
    public void MAQLearning(){
        ...
        //run 2500 learning episodes.You can comment out the visualiser paragraph for speed
        //However, we'll save each learning episode to disk so we can review them afterwards.

//        VisualWorldObserver wob = new VisualWorldObserver(domain, v);
//        wob.setFrameDelay(500);
//        wob.initGUI();
//        this.w.addWorldObserver(wob);
